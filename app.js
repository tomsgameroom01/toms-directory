...
onSnapshot(inventoryCollection, (snapshot) => {
  ...
  if (masterInventoryBody) {
    // If we're searching, apply filter; otherwise, render master dataset
    if (searchBox && searchBox.value.trim() !== "") {
      triggerSearchFilter(searchBox.value);
    } else {
      renderMasterTable(masterInventory);
    }
  }
});
...
function renderMasterTable(dataset) {
  ...
  if (dataset.length === 0) {
    masterInventoryBody.innerHTML = `<tr><td colspan="11" style="text-align:center; color:#7f8c8d; font-style:italic; padding: 20px;">No matching variant records found.</td></tr>`;
    return;
  }
...
function triggerSearchFilter(text) {
  ...
  const filtered = masterInventory.filter(
    (item) =>
      item.code.toLowerCase().includes(query) ||
      item.name.toLowerCase().includes(query) ||
      (item.color || "").toLowerCase().includes(query) ||
